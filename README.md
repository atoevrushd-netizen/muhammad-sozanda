...
На слабых устройствах (`isLowPower()` в `src/lib/viewStore.ts`):
- грузится **облегчённая модель** `city-mobile.glb` (текстуры 512 → вчетверо меньше видеопамяти);
- **полностью отключены** постобработка (bloom/godrays/SMAA) и тени — главные тормоза на мобильных GPU;
- низкий `dpr` (0.6–1.0) с **авто-снижением под нагрузкой** (AdaptiveDpr) — держит FPS;
- 1k HDRI вместо 2k, отключено стекло (transmission), дешёвый MSAA;
